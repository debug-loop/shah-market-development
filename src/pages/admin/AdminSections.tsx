...
export default function AdminSections() {
  ...
  const [formData, setFormData] = useState<SectionFormData>({
    ...
    description: '',
    order: 1,
    attributeSchema: JSON.stringify(defaultAttributeSchema, null, 2),
    isActive: true,
  });
  ...
  const handleOpenDialog = (section?: Section) => {
    if (section) {
      ...
      setFormData({
        ...
        order: section.order,
        attributeSchema: JSON.stringify(section.attributeSchema || {}, null, 2),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateSchema(formData.attributeSchema)) {
      return;
    }
    ...
    try {
      const submitData = {
        ...formData,
        attributeSchema: JSON.parse(formData.attributeSchema),
      };
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedSection
                ? 'Update section details and attribute schema'
                : 'Create a new marketplace section with custom attributes'}
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-2">
                <Label htmlFor="attributeSchema">
                  ...
                  <span className="text-xs text-muted-foreground ml-2">
                    Define custom attributes for products in this section
                  </span>
                </Label>
                <Textarea
                  id="attributeSchema"
                  ...
                  onChange={(e) => {
                    setFormData({ ...formData, attributeSchema: e.target.value });
                    setSchemaError('');
                  }}
                  onBlur={(e) => validateSchema(e.target.value)}
                  placeholder={JSON.stringify(defaultAttributeSchema, null, 2)}
                  rows={12}
                ...
                <p className="text-xs text-muted-foreground">
                  Example: quality (select), twoFa (boolean), country (string)
                </p>
              </div>
            </div>
            <DialogFooter>
